...
/**
  ...
  */
private[cloud] object JettyUtils extends Logging {
  ...
  def createStaticHandler(resourceBase: String, path: String): ServletContextHandler = {
    ...
    Option(Utils.getCloudClassLoader.getResource(resourceBase)) match {
      case Some(res) =>
    ...
    filters.foreach {
      case filter: String =>
        if (!filter.isEmpty) {
          ...
          conf.get("cloud." + filter + ".params", "").split(',').map(_.trim()).toSet.foreach {
            param: String =>
              if (!param.isEmpty) {
          ...
          val prefix = s"cloud.$filter.param."
          conf.getAll
  ...
  /**
    ...
    * @param conf         A CloudConf used to get the maximum number of retries when binding to a port.
...
private[cloud] case class ServerInfo(
                                      server: Server,
                                      boundPort: Int,
