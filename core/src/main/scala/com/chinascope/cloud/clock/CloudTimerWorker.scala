...
/**
  ...
  * @param workerType
  */
private[cloud] class CloudTimerWorker(name: String,
                                      var callback: () => Long,
  ...
  def updateCache(key: String, value: Long) {
    ...
    val nowStr = Utils.convertDateFormat(value)
    logDebug(Utils.convertDateFormat(new Date()) + ":\t update cache " + key + "->" + nowStr)
  }
  ...
  def monitorWorkerDaemon() {
    ...
    lastTimeCache match {
      case Some(lasttime) => logDebug(workerDaemonName + " run at " + Utils.convertDateFormat(lasttime))
      case None => {
  ...
  protected def monitorWorker() {
    ...
    lastTimeCache match {
      case Some(lasttime) => logDebug(workerName + " run at " + Utils.convertDateFormat(lasttime))
      case None => {
